...
class App extends Component {
  ...
  onDeleteHistory = id => {
    ...
    const filteredHistoryList = latestHistoryList.filter(
      eachItem => eachItem.id !== id,
    )

    this.setState({latestHistoryList: filteredHistoryList})
  }
  ...
  render() {
    ...
    if (latestHistoryList.length === 0 || searchResults.length === 0) {
      historyContainer = (
    ...
    return (
      <div className="main-container">
        ...
        <div className="history-container">{historyContainer}</div>
      </div>
    )
  }
}
